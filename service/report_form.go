	...
	for _, v := range param.ItemList {
		...
		if len(param.Statistics) == 0 {
			...
			for _, v1 := range param.Statistics {
				expr := fmt.Sprintf("%s_over_time((%s)[1440m:1m])", v1, pql)
				result := s.prometheus.QueryRange(expr, strconv.Itoa(param.Start), strconv.Itoa(param.End), strconv.Itoa(param.Step)).Data.Result
				if v1 == "max" {
			...
			for _, v1 := range result {
				for i, v2 := range v1.Values {
					...
					if len(maxResult[v1.Metric["instance"]].Values) != 0 {
						f.MaxValue = maxResult[v1.Metric["instance"]].Values[i][1].(string)
					}
					if len(minResult[v1.Metric["instance"]].Values) != 0 {
						f.MinValue = minResult[v1.Metric["instance"]].Values[i][1].(string)
					}
					if len(avgResult[v1.Metric["instance"]].Values) != 0 {
						f.AvgValue = avgResult[v1.Metric["instance"]].Values[i][1].(string)
					}
					reportForm = append(reportForm, f)
				}
			}
