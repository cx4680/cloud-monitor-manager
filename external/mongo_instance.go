...
type MongoAdditional struct {
}

func (s *MongoInstanceService) ConvertRealForm(f service.InstancePageForm) interface{} {
	param := InstanceRequest{
		CloudProductCode: "NDB",
		ResourceTypeCode: "mongo",
...
func (s *MongoInstanceService) DoRequest(url string, f interface{}) (interface{}, error) {
...
func (s *MongoInstanceService) ConvertResp(realResp interface{}) (int, []service.InstanceCommonVO) {
	response := realResp.(InstanceResponse)
	var list []service.InstanceCommonVO
	if response.Data.Total > 0 {
		for _, d := range response.Data.List {
			var additional = &MongoAdditional{}
